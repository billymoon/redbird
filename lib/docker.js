...
function DockerModule(redbird, url) {
  ...
  var wildDomains = this.wildDomains = {};

  var _this = this;

  function registerIfNeeded(imageName, containerId, containerName) {
    Object.keys(wildDomains).map(suffix => containerName + suffix).forEach(src => {
      if (!named[src]) {
        named[src] = true
        _this.registerContainer(src, containerId, {})
      }
    })
...
DockerModule.prototype.wild = function (wildDomainSuffix) {
  this.wildDomains[wildDomainSuffix] = true
}

DockerModule.prototype.tame = function (wildDomainSuffix) {
  this.wildDomains[wildDomainSuffix] = false
}
...
DockerModule.prototype.registerContainer = function (src, containerId, opts) {
  ...
  containerPort(this.dolphin, containerId).then(function (targetPort) {
    ...
    if (err.message.indexOf('No valid address or port undefined:undefined') !== 0) {
      throw err
    }
  });
};
...
function containerPort(dolphin, containerId) {
  return dolphin.containers.inspect(containerId).then(function (container) {
    // expose the lowest port number
    var port = Object.keys(container.NetworkSettings.Ports).map(item => item.split('/')[0] * 1).sort().reverse()[0]
