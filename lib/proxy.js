...
function ReverseProxy(opts) {
  ...
  if (opts.cluster && cluster.isMaster) {
    ...
    //
    // var proxy = this.proxy = httpProxy.createProxyServer(basic, {
...
ReverseProxy.prototype.setupHttpProxy = function (proxy, websocketsUpgrade, log, opts) {
  ...
  var server = this.server = httpServerModule.createServer(function (req, res) {
    ...
    // if (credentials && 'admin' === credentials.name && 'admin' === credentials.pass) {
    //   var src = _this._getSource(req);
    ...
    //   });
    // } else {
    //   console.log('getting bad...', credentials)
    //   res.statusCode = 401;
    //   res.setHeader('WWW-Authenticate', 'Basic realm="thing 1"');
...
ReverseProxy.prototype.setupHttpsProxy = function (proxy, websocketsUpgrade, log, sslOpts){
  ...
  var httpsServer = this.httpsServer = https.createServer(ssl, function (req, res) {
    ...
    if (credentials && 'admin' === credentials.name && process.env.EXTERNAL_PASSWORD === credentials.pass) {
        var src = _this._getSource(req);
        ...
        _this._getTarget(src, req).then(function (target) {
          if (target) {
            ...
            res.setHeader = (name, value) => { if (name.toLowerCase() !== 'www-authenticate') { setHeader.apply(res, [name, value]) } }
